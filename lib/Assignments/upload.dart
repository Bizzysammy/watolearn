...
class UploadsState extends State<Uploads> {
  ...
  Future<String> uploadPdf(String fileName, File file) async {
    final reference =
    FirebaseStorage.instance.ref().child("file/$fileName.pdf");
  ...
  void pickFile() async {
    ...
    if (pickedFile != null) {
      ...
      await _firebaseFirestore.collection("file").add({
        "name": fileName,
        "url": downloadlink,
      });
      // ignore: avoid_print
  ...
  void getAllPdf() async {
    final results = await _firebaseFirestore.collection("file").get();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('UPLOAD ASSIGNMENTS'),
      ),
